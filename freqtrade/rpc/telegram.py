        ...
        try:
            ...
            for r in results:
                lines = [
                    ...
                    ("*Minutes:* `{open_date_minutes}`" if r['open_date_minutes'] is not None else ""),
                    "*Current Pair:* {pair}",
                    "*Amount:* `{amount} ({stake_amount} {base_currency})`",
                    ("*Fibonacci retracement:* `{fibonnacci:.3f}`" if r['fibonnacci'] is not None else ""),
                    ("*support:* `{support:.8f}`" if r['support'] is not None else ""),
                    ("*resistance:* `{resistance:.8f}`" if r['resistance'] is not None else ""),
                    "*Open Rate:* `{open_rate:.8f}`",
